...
// Always add mode switch functionality
t.modeButton=new Phaser.Button(game,d-5,-60,"menuButton",(function(){
    console.log("Mode switch button clicked!");
    
    // Initialize enhanced difficulty if not available
    if (!window.enhancedDifficulty && window.EnhancedDifficulty) {
        window.enhancedDifficulty = new window.EnhancedDifficulty();
    }
    
    if (window.enhancedDifficulty) {
        var modes = ['classic', 'practice', 'aiBrain'];
        var currentIndex = modes.indexOf(window.enhancedDifficulty.currentMode);
        var nextMode = modes[(currentIndex + 1) % modes.length];
        
        console.log("Switching from", window.enhancedDifficulty.currentMode, "to", nextMode);
        window.enhancedDifficulty.switchMode(nextMode);
        
        // Update display
        switch(nextMode) {
            case 'practice':
                t.ratingText.text = projectInfo.practiceLevel.toString() + "P";
                t.modeLabel.text = "Practice Mode";
                break;
            case 'aiBrain':
                t.ratingText.text = projectInfo.aiBrainLevel.toString() + "B";
                t.modeLabel.text = "AI Brain Mode";
                break;
            default:
                t.ratingText.text = projectInfo.aiRating.toString();
                t.modeLabel.text = "Classic Mode";
        }
    } else {
        // Fallback: just cycle through display modes
        if (!t.currentMode) t.currentMode = 'classic';
        var modes = ['classic', 'practice', 'aiBrain'];
        var currentIndex = modes.indexOf(t.currentMode);
        t.currentMode = modes[(currentIndex + 1) % modes.length];
        
        console.log("Fallback mode switching to", t.currentMode);
...
t.modeButton.anchor=new Point(.5,.5),
t.modeButton.scale=new Point(.8,.8),
t.settingsWindow.addChild(t.modeButton),

// Always add mode label
var initialMode = (window.enhancedDifficulty && window.enhancedDifficulty.currentMode) ? 
                  (window.enhancedDifficulty.currentMode === 'practice' ? "Practice Mode" : 
                   window.enhancedDifficulty.currentMode === 'aiBrain' ? "AI Brain Mode" : "Classic Mode") : "Classic Mode";
t.modeLabel=new Phaser.BitmapText(game,d-5,-90,"font7",initialMode,28),
t.modeLabel.anchor=new Point(.5,.5),
t.settingsWindow.addChild(t.modeLabel),t.plusButton=new Phaser.Button(game,d+75,12,"plusButton",(function(){
    // Determine current mode
    var currentMode = (window.enhancedDifficulty && window.enhancedDifficulty.currentMode) || t.currentMode || 'classic';
    
    if(currentMode === 'practice') {
        if(projectInfo.practiceLevel < 100) {
            projectInfo.practiceLevel++;
            if(window.enhancedDifficulty) window.enhancedDifficulty.setPracticeLevel(projectInfo.practiceLevel);
            try{window.famobi.localStorage.setItem("practiceLevel",projectInfo.practiceLevel)}catch(e){}
            t.ratingText.text = projectInfo.practiceLevel.toString() + "P";
        ...
        if(projectInfo.aiBrainLevel < 110) {
            projectInfo.aiBrainLevel++;
            if(window.enhancedDifficulty) window.enhancedDifficulty.setAiBrainLevel(projectInfo.aiBrainLevel);
            try{window.famobi.localStorage.setItem("aiBrainLevel",projectInfo.aiBrainLevel)}catch(e){}
            t.ratingText.text = projectInfo.aiBrainLevel.toString() + "B";
