...
// frontend/public/8ball-match-withai/assets/src/99match-integration.js
(function () {
  ...
  const config = {
    ...
    winnerPrize: Number(params.get('winnerPrize') || params.get('prize') || 0),
    ai: Number(params.get('ai') || 3),
  ...
  const state = {
    ...
    isOnlineMatch: config.mode === 'match' || config.mode === 'ai' || config.mode === 'with_ai',
    socketReady: false,
  ...
  function ensureGameOverModal() {
    const existing = document.getElementById('ai-gameover-modal');
    if (existing) return existing;

    const style = document.createElement('style');
    style.textContent = [
      '#ai-gameover-modal{position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(6,8,12,0.72);z-index:9999;font-family:Arial,sans-serif;}',
      '#ai-gameover-modal .modal{background:#0f1724;color:#f8fafc;border-radius:18px;padding:24px 24px 18px;width:min(720px,92vw);box-shadow:0 20px 60px rgba(0,0,0,0.45);}',
      '#ai-gameover-modal .header{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;}',
      '#ai-gameover-modal .title{font-size:22px;font-weight:700;letter-spacing:0.3px;}',
      '#ai-gameover-modal .subtitle{font-size:14px;color:#94a3b8;margin-top:4px;}',
      '#ai-gameover-modal .close{background:transparent;border:none;color:#94a3b8;font-size:22px;cursor:pointer;}',
      '#ai-gameover-modal .row{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:8px;}',
      '#ai-gameover-modal .card{background:#111b2b;border:1px solid rgba(148,163,184,0.15);border-radius:16px;padding:16px;display:flex;gap:14px;align-items:center;}',
      '#ai-gameover-modal .card.winner{border-color:rgba(34,197,94,0.6);box-shadow:0 0 0 1px rgba(34,197,94,0.3) inset;}',
      '#ai-gameover-modal .avatar{width:64px;height:64px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,255,255,0.15);}',
      '#ai-gameover-modal .meta{display:flex;flex-direction:column;gap:6px;}',
      '#ai-gameover-modal .name{font-size:18px;font-weight:600;}',
      '#ai-gameover-modal .score{font-size:14px;color:#cbd5f5;}',
      '#ai-gameover-modal .meta-row{display:flex;justify-content:space-between;align-items:center;margin-top:14px;padding:10px 12px;border-radius:12px;background:#0b1220;color:#cbd5f5;font-size:14px;}',
      '#ai-gameover-modal .footer{display:flex;justify-content:flex-end;gap:10px;margin-top:18px;}',
      '#ai-gameover-modal .btn{background:#2563eb;color:#fff;border:none;border-radius:10px;padding:10px 16px;font-weight:600;cursor:pointer;}',
      '#ai-gameover-modal .btn.secondary{background:#1f2937;color:#e2e8f0;}'
    ].join('');
    document.head.appendChild(style);

    const overlay = document.createElement('div');
    overlay.id = 'ai-gameover-modal';
    overlay.innerHTML = [
      '<div class="modal">',
      '  <div class="header">',
      '    <div>',
      '      <div class="title">Match Result</div>',
      '      <div class="subtitle" data-winner=""></div>',
      '    </div>',
      '    <button class="close" type="button">&times;</button>',
      '  </div>',
      '  <div class="row">',
      '    <div class="card" data-player="p1">',
      '      <img class="avatar" alt="player1" />',
      '      <div class="meta">',
      '        <div class="name" data-name="p1"></div>',
      '        <div class="score" data-score="p1"></div>',
      '      </div>',
      '    </div>',
      '    <div class="card" data-player="p2">',
      '      <img class="avatar" alt="player2" />',
      '      <div class="meta">',
      '        <div class="name" data-name="p2"></div>',
      '        <div class="score" data-score="p2"></div>',
      '      </div>',
      '    </div>',
      '  </div>',
      '  <div class="meta-row">',
      ...
      '  </div>',
      '  <div class="footer">',
      '    <button class="btn secondary" type="button" data-action="lobby">Back to Lobby</button>',
    ...
    const lobbyBtn = overlay.querySelector('[data-action="lobby"]');
    const hide = () => {
  ...
  function showCustomGameOver(payload) {
    const modal = ensureGameOverModal();
    const winner = payload && payload.winner ? payload.winner : null;
    const p1Name = config.player1Name || 'Player 1';
    const p2Name = config.player2Name || 'AI';
    const p1Score = payload && payload.p1Score != null ? payload.p1Score : state.scores.p1;
    const p2Score = payload && payload.p2Score != null ? payload.p2Score : state.scores.p2;
    const prizeAmount = payload && payload.prizeAmount != null ? payload.prizeAmount : config.winnerPrize;
    const p1Avatar = config.player1Avatar || createAvatarPlaceholder(getPlayerInitials(p1Name), 128, '#2563eb', '#ffffff');
    const p2Avatar = config.player2Avatar || createAvatarPlaceholder(getPlayerInitials(p2Name), 128, '#f97316', '#ffffff');

    const p1Card = modal.querySelector('[data-player="p1"]');
    const p2Card = modal.querySelector('[data-player="p2"]');
    p1Card.classList.toggle('winner', winner === 'p1');
    p2Card.classList.toggle('winner', winner === 'p2');

    modal.querySelector('[data-name="p1"]').textContent = p1Name;
    modal.querySelector('[data-name="p2"]').textContent = p2Name;
    modal.querySelector('[data-score="p1"]').textContent = 'Score: ' + p1Score;
    modal.querySelector('[data-score="p2"]').textContent = 'Score: ' + p2Score;
    const winnerLabel = modal.querySelector('[data-winner]');
    if (winnerLabel) {
      const winnerName = winner === 'p1' ? p1Name : winner === 'p2' ? p2Name : '';
      winnerLabel.textContent = winnerName ? 'Winner: ' + winnerName : 'Match result';
    }
    const prizeLabel = modal.querySelector('[data-prize]');
    if (prizeLabel) {
      if (prizeAmount && prizeAmount > 0) {
        prizeLabel.textContent = 'Winner Prize: TSH ' + Number(prizeAmount).toLocaleString();
        prizeLabel.style.display = 'block';
      } else {
    ...
    const p1Img = modal.querySelector('[data-player="p1"] .avatar');
    const p2Img = modal.querySelector('[data-player="p2"] .avatar');
    if (p1Img) p1Img.src = p1Avatar;
    if (p2Img) p2Img.src = p2Avatar;

    modal.style.display = 'flex';
  }

  window.showCustomGameOver = showCustomGameOver;
  ...
  function applyGameState(gameState) {
    ...
    if (window.projectInfo) {
      if (typeof clientState.p1Score === 'number') {
        window.projectInfo.p1Score = clientState.p1Score;
        state.scores.p1 = clientState.p1Score;
      }
      if (typeof clientState.p2Score === 'number') {
        window.projectInfo.p2Score = clientState.p2Score;
        state.scores.p2 = clientState.p2Score;
      }
      if (typeof clientState.timeRemainingSeconds === 'number') {
        window.projectInfo.matchTimeRemainingSeconds = clientState.timeRemainingSeconds;
      }
    }
    if (gameState.matchStartTime) {
      state.matchStartTime = new Date(gameState.matchStartTime);
    }
    if (typeof gameState.maxDurationSeconds === 'number') {
      state.maxDurationSeconds = gameState.maxDurationSeconds;
    }
  ...
  function sendShot(gameInfo) {
    if (!state.gameSocket || !state.socketReady || !gameInfo) {
      console.log('[8Ball] Shot blocked: socket not ready', {
    ...
    // More lenient turn check - if turn is not set yet, allow the shot (server will validate)
    if (state.localSide && gameInfo.turn && gameInfo.turn !== state.localSide) {
      console.log('[8Ball] Shot blocked: not player turn', {
        localSide: state.localSide,
    ...
    if (!gameInfo.aimDirectionVector) {
      console.log('[8Ball] Shot blocked: no aim direction');
      return;
    }
    ...
    console.log('[8Ball] Sending shot:', payload);
  ...
  function getMatchTiming(gameInfo) {
    const resolvedGameInfo = gameInfo || window.playState?.gameInfo || null;
    ...
    const gameElapsedSeconds = Math.floor((resolvedGameInfo?.time || 0) / 60);
  ...
  function finalizeMatch(gameInfo, reason) {
    ...
    if (window.showCustomGameOver) {
      window.showCustomGameOver({
        winner,
        p1Score: state.scores.p1,
        p2Score: state.scores.p2,
        prizeAmount: config.winnerPrize || 0
      });
    }
  }
  ...
  function loadSocketIo(url, socketPath, onReady) {
    ...
    const localSrc = '/8ball-match-withai/assets/lib/socket.io.min.js';
  ...
  function connectGameSession() {
    ...
    if (!state.gameSocket) {
      ...
      state.gameSocket.on('game:completed', (data) => {
        ...
        if (window.showCustomGameOver) {
          const winner = data?.winnerId === config.player1Id ? 'p1' : (data?.winnerId === config.player2Id ? 'p2' : null);
          window.showCustomGameOver({
            winner,
            p1Score: data?.player1Score || 0,
            p2Score: data?.player2Score || 0,
            prizeAmount: config.winnerPrize || 0
          });
        }
      });
  ...
  function tick() {
    ...
    const { elapsedSeconds, remainingSeconds } = getMatchTiming(gameInfo);

    if (window.projectInfo) {
      window.projectInfo.p1Score = state.scores.p1;
      window.projectInfo.p2Score = state.scores.p2;
      window.projectInfo.matchDurationSeconds = state.maxDurationSeconds;
      window.projectInfo.matchTimeRemainingSeconds = remainingSeconds;
    }
  ...
  window.addEventListener('message', (event) => {
    ...
    if (type === 'SET_PLAYER_DATA' && data) {
      ...
      if (typeof data.winnerPrize === 'number') {
        config.winnerPrize = data.winnerPrize;
      }
  ...
  window.POOL_DEBUG = {
    ...
    timing: () => getMatchTiming(window.playState?.gameInfo)
  };
