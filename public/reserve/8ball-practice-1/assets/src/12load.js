...
loadState.init = function() {
    ...
    e.loaderHighlight.anchor = new Point(.5, 0);
    ...
    this.resizeGame();
    game.add.tween(e.loaderCanvas).to({alpha: 1}, 1e3, Phaser.Easing.Linear.None, true, 500);
};

loadState.resizeGame = function(e, s) {
    var t = this.loaderInfo;
    
    // Guard against resize calls after the loader has been torn down.
    if (!t || !t.loaderCanvas) {
        return;
    }
    ...
    if ("portrait" == window.famobi.getOrientation()) {
        ...
        if (t.title) {
            t.title.y = -game.height / 2 * .5;
        }
    } else {
        game.scale.setGameSize(1920, 1080);
        if (t.title) {
            t.title.y = -game.height / 2 * .6;
        }
    }
    
    // ✅ Safe to access loaderCanvas now - but double-check for race conditions
    if (t.loaderCanvas) {
    ...
    if (t.loaderBase) {
        t.loaderBase.y = .6 * game.height / 2;
    }
    
    if (t.loaderFill) {
        t.loaderFill.y = .6 * game.height / 2;
    }
    
    if (t. loaderHighlight) {
        t.loaderHighlight.y = .6 * game.height / 2;
    }
    
    if (t.rack) {
        t.rack.y = game.height / 2 * .1;
    }
    
    if (t.graphics) {
        t.graphics. clear();
        t.graphics.beginFill(16777215);
        var o = t.loaderBase ?  t.loaderBase.y : 0;
        t. graphics.drawRect(game.width / 2 - 187.5, game.height / 2 + o + 8, 3.75 * t.loaderProgress, 30);
    }
};
...
loadState.updateProgressBar = function() {
    ...
    if (!e || !e.loaderCanvas || ! e.graphics || !e.loaderBase) {
        return;
    }
    ...
    e.graphics.clear();
    e.graphics.beginFill(16777215);
    e.graphics.drawRect(game.width / 2 - 187.5, game.height / 2 + e.loaderBase.y + 8, 3.75 * e.loaderProgress, 30);
};
...
loadState.create = function() {
    ...
    game.time.events.add(2 * Phaser.Timer.SECOND, function() {
        if (e && e.loaderCanvas) {
            game.add.tween(e.loaderCanvas).to({alpha: 0}, 1e3, Phaser.Easing.Linear. None, true).onComplete.add(s, this);
        }
    }, this);
};
...
loadState.shutdown = function() {
    ...
    if (e && e.loaderCanvas) {
        game.stage.removeChild(e.loaderCanvas);
        e.loaderCanvas = null;
    }
